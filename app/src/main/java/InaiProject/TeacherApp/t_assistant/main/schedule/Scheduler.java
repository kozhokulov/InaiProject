package InaiProject.TeacherApp.t_assistant.main.schedule;
...
public class Scheduler extends AppCompatActivity implements AdapterView.OnItemLongClickListener {
    ...
    private void loadSchedules() {
        ...
        if (cursor == null || cursor.getCount() == 0) {
            Toast.makeText(getBaseContext(), "Нет доступного расписания", Toast.LENGTH_LONG).show();
        } else {
            cursor.moveToFirst();
            while (!cursor.isAfterLast()) {
                subx.add(cursor.getString(1));
                subs.add(cursor.getString(1) + "\nдля " + cursor.getString(0) + "\nв " + cursor.getString(2) + " : " + cursor.getString(3));
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
        ...
        alert.setTitle("Удалить расписание?");
        alert.setMessage("Вы хотите удалить расписание ?");
